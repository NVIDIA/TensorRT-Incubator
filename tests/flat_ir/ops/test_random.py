...
class TestRandomOp:
    def test_uniform_str(self):
        ...
        assert (
            str(random)
            == "out: [shape=(2, 3,), dtype=(float32), loc=(gpu:0)] = RandomUniformOp(static_low=0.0, static_high=1.0)"
        )
    ...
    def test_normal_str(self):
        ...
        assert (
            str(random)
            == "out: [shape=(2, 3,), dtype=(float32), loc=(gpu:0)] = RandomNormalOp(static_mean=0.0, static_std=1.0)"
        )
