...
class TestDynamic:
    ...
    def test_repr(self, range):
        ...
        dim = tp.dynamic_dim(2, max=range)
        if dim.is_dynamic_dim():
            assert repr(dim) == f"dynamic_dim(runtime_value=2, min=2, opt=2, max={range})"
            from tripy import dynamic_dim  # Required to make eval() work
    ...
    def test_profile_dict(self, dim_range):
        dim = tp.dynamic_dim(runtime_value=3, **dim_range)
    ...
    def test_profile_out_of_range(self):
        with pytest.raises(TripyException):
            _ = tp.dynamic_dim(2, min=3)

    def test_validate_set_runtime_value(self):
        dim = tp.dynamic_dim(runtime_value=3, min=2, opt=4, max=6)
        dim.runtime_value = 5
