...
class TestConvolution:
    ...
    def test_convolution_1d(self, torch_dtype, tp_dtype, tp_pad, torch_pad, stride, groups):
        ...
        conv_layer_torch = torch.nn.Conv1d(
            4, 8, 3, bias=False, padding=torch_pad, stride=stride, groups=groups, dtype=torch.float32
        )
        conv_layer_torch.requires_grad = False
        conv_layer = tp.Conv(4, 8, (3,), padding=tp_pad, stride=stride, groups=groups, dtype=tp_dtype)
    ...
    @pytest.mark.parametrize("groups", [1, 2, 4])
    def test_convolution_2d(self, torch_dtype, tp_dtype, tp_pad, torch_pad, stride, groups):
        ...
        conv_layer_torch.requires_grad = False
        conv_layer = tp.Conv(4, 8, (3, 3), padding=tp_pad, stride=stride, groups=groups, dtype=tp_dtype)
        conv_layer.weight = tp.cast(tp.Tensor(conv_layer_torch.weight.data), tp_dtype)
    ...
    def test_convolution_3d(self, torch_dtype, tp_dtype, tp_pad, torch_pad, stride, groups):
        ...
        conv_layer_torch.requires_grad = False
        conv_layer = tp.Conv(4, 8, (3, 3, 3), padding=tp_pad, stride=stride, groups=groups, dtype=tp_dtype)
        conv_layer.weight = tp.cast(tp.Tensor(conv_layer_torch.weight.data), tp_dtype)
    ...
    @pytest.mark.parametrize("stride", [(2, 1), (1, 1)])
    @pytest.mark.parametrize("groups", [1, 2, 4])
    def test_uneven_padding(self, torch_dtype, tp_dtype, stride, groups):
        input_torch = torch.randn((2, 4, 5, 5), dtype=torch.float32)
        input = tp.cast(tp.Tensor(input_torch), tp_dtype)
        ...
        conv_layer_torch = torch.nn.Conv2d(
            4, 8, 3, bias=False, padding=0, stride=stride, groups=groups, dtype=torch.float32
        )
        conv_layer_torch.requires_grad = False
        conv_layer = tp.Conv(4, 8, (3, 3), padding=tp_pad, stride=stride, groups=groups, dtype=tp_dtype)
        conv_layer.weight = tp.cast(tp.Tensor(conv_layer_torch.weight.data), tp_dtype)
