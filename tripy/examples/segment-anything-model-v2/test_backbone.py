...
def test_trunk():
    ...
    compiled_tp_trunk = tp.compile(
        ...
        args=[
            tp.InputInfo(((1, 2, 4), 3, 1024, 1024), dtype=tp.float32),
        ],
    )
...
def test_neck():
    ...
    compiled_tp_neck = tp.compile(
        ...
        optimization_level=3,
        args=[
            tp.InputInfo(((1, 2, 8), 144, 256, 256), dtype=tp.float32),
            tp.InputInfo(((1, 2, 8), 288, 128, 128), dtype=tp.float32),
            tp.InputInfo(((1, 2, 8), 576, 64, 64), dtype=tp.float32),
            tp.InputInfo(((1, 2, 8), 1152, 32, 32), dtype=tp.float32),
        ],
    )
...
def test_dynamic_pooling():
    def model(x):
        q_stride = (2, 2)
        x = tp.maxpool(x, kernel_dims=q_stride, stride=q_stride)
        return x
    ...
    compiled_model = tp.compile(
        ...
        optimization_level=3,
        args=[
            tp.InputInfo(((1, 2, 8), 3, 1024, 1024), dtype=tp.float32),
        ],
    )
