...
import tripy as tp
from sam2.modeling.backbones_tripy.utils import (
    PatchEmbed,
    window_partition,
    window_unpartition,
)
from sam2.modeling.sam2_utils import tripy_MLP, tripy_scaled_dot_product_attention
...
class MultiScaleAttention(tp.Module):
    ...
    def __call__(self, x):
        ...
        x = tripy_scaled_dot_product_attention(
            tp.transpose(q, 1, 2),
            tp.transpose(k, 1, 2),
            tp.transpose(v, 1, 2),
        )
...
class Hiera(tp.Module):
    ...
    def __init__(
        ...
        self.pos_embed = tp.Parameter(
            tp.zeros(
                (1, embed_dim, *self.window_pos_embed_bkg_spatial_size), dtype=tp_dtype
            )
        )
        self.pos_embed_window = tp.Parameter(
            tp.zeros(
                (1, embed_dim, self.window_spec[0], self.window_spec[0]), dtype=tp_dtype
            )
        )
    ...
    def _get_pos_embed(self, hw: Tuple[int, int]) -> tp.Tensor:
        ...
        pos_embed = tp.resize(
            self.pos_embed, mode="cubic", output_shape=pos_embed_shape
        )
        # WAR: tp.repeat twice
    ...
    def _get_pos_embed_torch(self, hw: Tuple[int, int]):
        ...
        pos_embed = F.interpolate(
            torch.from_dlpack(self.pos_embed), size=(h, w), mode="bicubic"
        )
