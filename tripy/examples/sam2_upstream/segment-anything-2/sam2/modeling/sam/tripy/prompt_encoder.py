...
import tripy as tp
from sam2.modeling.tripy.position_encoding import PositionEmbeddingRandom
...
class PromptEncoder(tp.Module):
    def __init__(
        ...
        self.point_embeddings = [
            tp.Embedding(1, embed_dim) for i in range(self.num_point_embeddings)
        ]
        ...
        self.mask_downscaling = [
            ...
            activation,
            tp.Conv(
                mask_in_chans // 4, mask_in_chans, kernel_dims=(2, 2), stride=(2, 2)
            ),
    ...
    def forward(
        ...
        if points is not None:
            ...
            sparse_embeddings = tp.concatenate(
