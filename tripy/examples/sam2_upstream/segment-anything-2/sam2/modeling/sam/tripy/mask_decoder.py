...
from sam2.modeling.tripy.sam2_utils import LayerNorm2d, MLP
...
class MaskDecoder(tp.Module):
    def __init__(
        ...
        *,
        ...
        self.iou_token = tp.Embedding(1, transformer_dim)
        self.num_mask_tokens = num_multimask_outputs + 1
        self.mask_tokens = tp.Embedding(self.num_mask_tokens, transformer_dim)

        self.pred_obj_scores = pred_obj_scores
        if self.pred_obj_scores:
            self.obj_score_token = tp.Embedding(1, transformer_dim)
        self.use_multimask_token_for_obj_ptr = use_multimask_token_for_obj_ptr

        self.output_upscaling = [
            tp.ConvTranspose(
                transformer_dim, transformer_dim // 4, kernel_dims=(2, 2), stride=(2, 2)
            ),
        ...
        if use_high_res_features:
            self.conv_s0 = tp.Conv(
                transformer_dim, transformer_dim // 8, kernel_dims=(1, 1), stride=(1, 1)
            )
            self.conv_s1 = tp.Conv(
                transformer_dim, transformer_dim // 4, kernel_dims=(1, 1), stride=(1, 1)
            )
    ...
    def predict_masks(
        ...
        output_tokens = tp.expand(
            tp.unsqueeze(output_tokens, 0), (sparse_prompt_embeddings.shape[0], -1, -1)
        )
    ...
    def _get_stability_scores(self, mask_logits):
        ...
        """
        mask_logits = tp.flatten(mask_logits.flatten, -2)
        stability_delta = self.dynamic_multimask_stability_delta
        area_i = tp.cast(tp.sum(mask_logits > stability_delta, dim=-1), tp.float32)
        area_u = tp.cast(tp.sum(mask_logits > -stability_delta, dim=-1), tp.float32)
    ...
    def _dynamic_multimask_via_stability(self, all_mask_logits, all_iou_scores):
        ...
        batch_inds = tp.arange(multimask_iou_scores.shape[0])
        ...
        is_stable = tp.unsqueeze(tp.unsqueeze(is_stable, 1), 2)
