...
def _hstu_attention_maybe_from_cache(
    ...
    if delta_x_offsets is not None:
        ...
        if verbose:
            print(f"          ↩️  Conversion #2: Q,K Jagged→Dense {q.shape} → ", end="")
    ...
    if verbose:
        print(f"          ↩️  Conversion #3: V Jagged→Dense {v.shape} → ", end="")
    padded_v = torch.ops.fbgemm.jagged_to_padded_dense(v, [x_offsets], [n])
    if verbose:
        print(f"{padded_v.shape}")
    ...
    if verbose:
        print(f"          🔀 Conversion #4: Attn Dense→Jagged {attention_result.shape} → ", end="")
...
class SequentialTransductionUnitJagged(torch.nn.Module):
    ...
    def forward(  # pyre-ignore [3]
        ...
        if self._normalization == "rel_bias" or self._normalization == "hstu_rel_bias":
            ...
            if self._verbose:
                print(f"          🎯 Calling attention with relative bias ({self._normalization})")
            ...
            if delta_x_offsets is not None:
                ...
                print(f"            🔄 TENSOR CONVERSION #3.2: jagged_to_padded_dense (Q, K for attention)")
                print(f"              Before: q {q.shape}, k {k.shape} (jagged)")
                padded_q = torch.ops.fbgemm.jagged_to_padded_dense(
                    values=q, offsets=[x_offsets], max_lengths=[n], padding_value=0.0
                )
                padded_k = torch.ops.fbgemm.jagged_to_padded_dense(
                ...
                print(f"              After: padded_q {padded_q.shape}, padded_k {padded_k.shape} (dense)")
...
class HSTUJagged(torch.nn.Module):
    ...
    def jagged_forward(
        ...
        """
        if self._verbose:
            print(f"      🔄 Processing {len(self._attention_layers)} attention layers...")
    ...
    def forward(
        ...
        """
        if self._verbose:
            print(f"    🏗️  HSTUJagged processing...")

        if len(x.size()) == 3:
            if self._verbose:
                print(f"    🔀 Conversion #1: Dense→Jagged {x.shape} → ", end="")
        ...
        if self._verbose:
            print(f"    ↩️  Conversion #FINAL: Jagged→Dense {jagged_x.shape} → ", end="")
...
class HSTU(SequentialEncoderWithLearnedSimilarityModule):
    ...
    def generate_user_embeddings(
        ...
        # Create x_offsets for jagged tensor operations
        x_offsets = torch.ops.fbgemm.asynchronous_complete_cumsum(past_lengths)

        if self._verbose:
            print(f"  🔢 Offset computation: {past_lengths.shape} → {x_offsets.shape}")
        ...
        result = self._output_postproc(user_embeddings), cached_states
    ...
    def forward(
        ...
        """
        if self._verbose:
            print(f"🚀 HSTU.forward() → generate_user_embeddings()")
        ...
        if self._verbose:
            print(f"✅ HSTU.forward() complete: {encoded_embeddings.shape}")
    ...
    def encode(
        ...
        """
        if self._verbose:
            print(f"🎯 HSTU.encode() → extracting current embeddings")
        ...
        if self._verbose:
            if return_cache_states:
                print(f"✅ HSTU.encode() complete: {result[0].shape}")
            else:
                print(f"✅ HSTU.encode() complete: {result.shape}")
