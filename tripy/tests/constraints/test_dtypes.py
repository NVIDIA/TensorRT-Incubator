...
for api in PUBLIC_APIS:
    ...
    for func in funcs:
        if any(tensor_type in str(inspect.signature(func)) for tensor_type in ["Tensor", "Shape", "ShapeScalar"]):
...
def test_all_public_apis_verified(api):
    NON_VERIFIABLE_APIS = {
        "plugin",
        "Executable.__call__",
        # TODO: Remove these two exceptions once they disallow non-Shape types
        "Shape.add",
        "Shape.multiply",
    }
