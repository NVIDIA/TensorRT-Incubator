...
class FlatIRExecutor:
    ...
    def __init__(
        self, executable: void_ptr, output_devices: List, input_types: List = None, output_types: List = None
    ) -> None:
        """
        Args:
            executable: mlir executable generated by the compiler
            output_devices: output devices inferred by Trace
            input_types: Shape and data type info for inputs used to deduce runtime values
            output_types: Shape and data type info outputs used for output allocation
        """
    ...
    def execute(self, inputs: List[Tensor] = []) -> List[Storage]:
        ...
        exec_args = self.compiler.exec_initializer(
            self.executable, device_inputs, self.output_devices, self.input_types, self.output_types
        )
        ...
        num_outputs: int = len(self.output_types)
