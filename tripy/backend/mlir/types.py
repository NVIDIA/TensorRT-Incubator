...
class TensorShape(ctypes.Structure):
    ...
    def __init__(self, dtype: tripy.common.datatype.DataType = tripy.common.datatype.float32, shape: List = []):
        def _get_mlir_dtype(dtype: tripy.common.datatype.DataType) -> MlirDataType:
            """Get the data type of the tensor."""
            """Get the tripy.common.datatype equivalent of the data type."""
            if isinstance(dtype, MlirDataType):
                return dtype.value

            if isinstance(dtype, tripy.common.datatype.DataType):
                to_mlir = {
                    "float32": 0,
                    "float16": 1,
                    "int8": 2,
