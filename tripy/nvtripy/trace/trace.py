...
class Trace:
    ...
    def __str__(self) -> str:
        ...
        input_strs = []
        # TODO (pranavm): Fix string representation:
        for inp, inp_shape in zip(
            self.inputs,
            (
                [info.shape_bounds for info in self.input_infos.values()]
                if self.input_infos
                else [None] * len(self.inputs)
            ),
        ):
                    ...
                    for inp in self.inputs:
                        ...
                        if self.input_infos:
                            ...
                            attr["tensorrt.dimension_names"] = ir.DictAttr.get(
                                {
                                    # ir.IntegerAttr.get(ir.IntegerType.get_signless(32), idx): ir.StringAttr.get(name)
                                    # ir.StringAttr.get(str(idx)): ir.StringAttr.get(name)
