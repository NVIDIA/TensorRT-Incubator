...
from nvtripy.trace.ops.unary import Cos
...
def cos(input: "nvtripy.Tensor") -> "nvtripy.Tensor":
    ...
    .. code-block:: python
        ...
        assert tp.allclose(output, tp.Tensor(np.cos(np.from_dlpack(tp.copy(input, device=tp.device("cpu"))))))
    """
    return op_utils.create_op(Cos, [input])
