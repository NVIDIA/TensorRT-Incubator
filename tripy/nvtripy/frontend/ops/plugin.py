    ...
    **kwargs,
    ...
    for key, value in kwargs.items():
        if key not in trtp_op.input_attrs:
            raise_error(f"Unexpected attribute {key} provided. Expected one of {trtp_op.input_attrs.keys()}.")
        ...
        if isinstance(value, np.ndarray):
            ...
            if attr_annotation is not type(value):
                raise_error(f"Unexpected type '{type(value)}' for attribute '{key}'. Expected '{attr_annotation}'.")
