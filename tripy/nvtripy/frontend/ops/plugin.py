...
def plugin(
    ...
    **kwargs,
    ...
    """
    namespace, name = op.plugin_id.split("::")

    attrs = {}
    for key, value in kwargs.items():
        if key not in op.input_attrs:
            raise ValueError(
                f"Unexpected attribute {key} provided. Expected one of {op.input_attrs.keys()}."
            )

        attr_annotation = op.input_attrs[key]
    ...
    output_descs = op.register_func(*input_descs, attrs)
