...
def triu(tensor: "nvtripy.Tensor", diagonal: int = 0) -> "nvtripy.Tensor":
    ...
    .. code-block:: python
        ...
        assert np.array_equal(np.from_dlpack(tp.copy(output, device=tp.device("cpu"))), np.triu(np.from_dlpack(tp.copy(input, device=tp.device("cpu")))))
    ...
    .. code-block:: python
        ...
        assert np.array_equal(np.from_dlpack(tp.copy(output, device=tp.device("cpu"))), np.triu(np.from_dlpack(tp.copy(input, device=tp.device("cpu"))), 2))
    ...
    .. code-block:: python
        ...
        assert np.array_equal(np.from_dlpack(tp.copy(output, device=tp.device("cpu"))), np.triu(np.from_dlpack(tp.copy(input, device=tp.device("cpu"))), -1))
    """
