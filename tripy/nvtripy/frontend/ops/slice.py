...
from nvtripy.types import IntLike, TensorLike
...
def __getitem__(
    ...
    for dim_idx, (dim_size, slice_idx) in enumerate(zip(inp_shape, index)):
        ...
        if isinstance(slice_idx, int) or isinstance(slice_idx, DimensionSize):
            ...
            starts.append(start)
            # TODO (pranavm): Need `floor_div` and `abs` in order to be able to correctly compute `sizes`
            # when step is not 1. Maybe want to optimize for the case where it is 1?
            sizes.append((to_positive_idx(slice_idx.stop) if slice_idx.stop is not None else dim_size) - start)
            steps.append(utils.utils.default(slice_idx.step, 1))
