...
from nvtripy.trace.ops.binary import Div
...
def __truediv__(self: "nvtripy.Tensor", other: TensorLike) -> "nvtripy.Tensor":
    ...
    """
    return create_binary_op(Div, self, other)
...
def __rtruediv__(self: "nvtripy.Tensor", other: TensorLike) -> "nvtripy.Tensor":
    ...
    .. code-block:: python
        ...
        assert np.array_equal(np.from_dlpack(tp.copy(output, device=tp.device("cpu"))), np.array([3.0, 2.0]))
    """
    return create_binary_op(Div, other, self)
