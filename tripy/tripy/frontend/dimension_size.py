...
class DimensionSize(Tensor):
    ...
    def __init__(
        ...
        r"""
        Args:
            data: The value of the DimensionSize, which should be a scalar integer tensor.
        ...
        if data.trace_tensor.rank is not None and data.trace_tensor.rank != 0:
            raise_error(f"Scalar shape tensors must be of rank 0, but input tensor is rank {data.rank}", details=[data])
