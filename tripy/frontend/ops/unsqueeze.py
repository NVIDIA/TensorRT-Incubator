...
class Unsqueeze(BaseOperator):
    ...
    def to_flat_ir(self, inputs, outputs):
        from tripy.flat_ir.ops import BroadcastOp, ConstantOp, ConcatenateOp, DynamicBroadcastOp, SliceOp
        ...
        if dynamic_shape:
            ...
            concat_output_tensor = FIRTensor.build(
                shape=(Dim(1 + len(inputs[0].shape)),), dtype=int32, device=inputs[0].device
            )

            # Slice the first half of shape : shape[:dim]
            slice_first_half = FIRTensor.build(shape=(Dim(self.dim),), dtype=int32, device=inputs[0].device)
            ...
            slice_second_half = FIRTensor.build(
