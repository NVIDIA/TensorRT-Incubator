...
func.func @trt_select(%arg0: tensor<10x10xi1>, %arg1: tensor<1x10xf16>, %arg2: tensor<10x1xf32>) -> tensor<10x10xf32> {
  // expected-error @below {{'tensorrt.select' op failed to verify that all of {thenInput, elseInput, result} have same element type}}
...
func.func @trt_softmax(%arg0: tensor<10x10xi8>) -> tensor<10x10xi8> {
  // expected-error @below {{'tensorrt.softmax' op operand #0 must be 0D/1D/2D/3D/4D/5D/6D/7D/8D tensor of 16-bit float or bfloat16 type or 32-bit float values, but got 'tensor<10x10xi8>'}}
...
func.func @trt_matmul_unsupported_dtype(%arg0: tensor<5x10xi32>, %arg1: tensor<10x5xi32>) -> tensor<5x5xi32> {
  // expected-error @below {{'tensorrt.matrix_multiply' op operand #0 must be 0D/1D/2D/3D/4D/5D/6D/7D/8D tensor of 16-bit float or bfloat16 type or 32-bit float values, but got 'tensor<5x10xi32>'}}
  %0 = tensorrt.matrix_multiply {
    op0 = #tensorrt.matrix_operation<kNONE>,
    op1 = #tensorrt.matrix_operation<kNONE>
...
func.func @trt_scatter_nd_unexpected_indices_type(%arg0: tensor<4x4x4xf32>, %arg1: tensor<2x1xi8>, %arg2: tensor<2x4x4xf32>) -> tensor<4x4x4xf32> {
  // expected-error @below {{'tensorrt.scatter_nd' op operand #1 must be 0D/1D/2D/3D/4D/5D/6D/7D/8D tensor of 32-bit signless integer or 64-bit signless integer values, but got 'tensor<2x1xi8>'}}
  %0 = tensorrt.scatter_nd
    data(%arg0 : tensor<4x4x4xf32>)
