  ...
  LogicalResult matchAndRewrite(tensorrt::EinsumOp op,
    ...
    for (size_t i = 0; i < op.getInputs().size(); i++) {
      ...
      if (auto transpose = input.getDefiningOp<tensorrt::TransposeOp>()) {
        ...
        equation = perm.compose(equation);
        einsumEquationLhsParts[i] = "";
        for (int i = 0; i < equation.size(); i++) {
          einsumEquationLhsParts[i] += (char)equation[i];
        }
...
// the ordering of the output axes This should enable converting the einsum back
// to a matmul einsum(x1, x2, ...) -> einsum(x1, transpose(x2), ...)
  ...
  LogicalResult matchAndRewrite(tensorrt::ReshapeOp op,
    ...
    for (int i = 0, j = 0; i < reshapeOutShape.size(); i++) {
      if (reshapeOutShape[i] <= 1) {
        // if 0-shape, then means the tensor is empty.  Annoying edge case that
        // not going to handle
        // TODO: if 1-shape, then need additional logic to handle this
        return failure(); // TODO: handle? should this get pushed, it is the
                          // result of expand rank ops
      }
...
public:
  ...
  void runOnOperation() override {
...

    {
      ...
      patterns.insert<EinsumToMatrixMultiply>(ctx);
      if (failed(applyPatternsGreedily(op, std::move(patterns)))) {
        emitError(op->getLoc())
            << "failed to apply pushup patterns in " << getArgument();
        return signalPassFailure();
