...
//===----------------------------------------------------------------------===//
static void emitEnumSymbolizer(llvm::StringRef className, const Record *def,
                               raw_indented_ostream &os) {
  os << "/// Returns a " << className
     << " value corresponding to `str` if possible, otherwise returns "
        "nullopt.\n";
  os << "static inline std::optional<" << className << "> parse" << className
     << "(std::string_view str) {\n";
  os.indent();
...
//===----------------------------------------------------------------------===//
static void emitEnumStringifier(llvm::StringRef className, const Record *def,
                                raw_indented_ostream &os) {
  os << "/// Returns a string representation of " << className << ".\n";
  os << "static inline std::string_view stringify" << className << "("
     << className << " val) {\n";
  os.indent();
  ...
  for (const Record *enumCase : def->getValueAsListOfDefs("cases")) {
    if (enumCase->getValueAsString("symbol").contains("_SIZE"))
      continue;
    os << "case " << className << "::" << enumCase->getValueAsString("symbol")
       << ":\n";
    os.indent();
...
static bool emitEnumDefs(const llvm::RecordKeeper &recordKeeper,
...

  {
    ...
    for (const Record *def : defs) {
      ...
      emitEnumSymbolizer(className, def, os);
      os << "\n\n";
      emitEnumStringifier(className, def, os);
      os << "\n\n";
    }
  }
  return false;
