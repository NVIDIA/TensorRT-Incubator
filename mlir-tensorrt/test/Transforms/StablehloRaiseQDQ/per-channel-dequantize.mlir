...
func.func @dequantize_to_f32_static(%arg0: tensor<2x3x300x300xi8>) -> tensor<2x3x300x300xf32> {
  %cst = stablehlo.constant dense_resource<__elided__> : tensor<3xf32>
  %0 = stablehlo.convert %arg0 : (tensor<2x3x300x300xi8>) -> tensor<2x3x300x300xf32>
  %1 = stablehlo.broadcast_in_dim %cst, dims = [1] : (tensor<3xf32>) -> tensor<2x3x300x300xf32>
...
//   CHECK-NEXT: %[[v0:.+]] = stablehlo.constant dense_resource<__elided__> : tensor<3xf32>
//   CHECK-NEXT: %[[v1:.+]] = stablehlo.broadcast_in_dim %[[v0]], dims = [1] : (tensor<3xf32>) -> tensor<2x3x300x300xf32>
...
func.func @dequantize_to_f32_dynamic(%arg0: tensor<?x?x?x?xi8>) -> tensor<?x?x?x?xf32> {
  ...
  %10 = stablehlo.dynamic_broadcast_in_dim %cst, %9, dims = [1] : (tensor<3xf32>, tensor<4xi32>) -> tensor<?x?x?x?xf32>
...
func.func @dequantize_to_f32_eager() -> tensor<258x256xf32> {
  ...
  %cst = stablehlo.constant dense_resource<__elided__> : tensor<258xf32>
  %0 = stablehlo.convert %c : (tensor<258x256xi8>) -> tensor<258x256xf32>
  %1 = stablehlo.broadcast_in_dim %cst, dims = [0] : (tensor<258xf32>) -> tensor<258x256xf32>
