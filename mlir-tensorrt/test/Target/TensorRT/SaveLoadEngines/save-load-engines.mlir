...
// RUN: rm -rf %t || true
// RUN: mkdir %t
// RUN: %pick-one-gpu mlir-tensorrt-opt %s -pass-pipeline="builtin.module(tensorrt.module(translate-tensorrt-to-engine))" \
// RUN:  --mlir-elide-elementsattrs-if-larger=32 \
// RUN:  --save-tensorrt-engines=%t
// RUN: %pick-one-gpu mlir-tensorrt-opt %s -pass-pipeline="builtin.module(trensorrt.module(translate-tensorrt-to-engine))" \
// RUN:  --mlir-elide-elementsattrs-if-larger=32 \
