// RUN: mlir-tensorrt-opt -allow-unregistered-dialect -pass-pipeline="builtin.module(translate-tensorrt-to-engine)" -tensorrt-builder-opt-level=0 -split-input-file --verify-diagnostics %s

// Ensure that when the plugin creator returns a nullptr, we fail gracefully.
// expected-error @below {{failed to translate function 'test_opaque_v2_plugin_creation_failure' to a TensorRT engine}}
func.func @test_opaque_v2_plugin_creation_failure(%arg0: tensor<1xf32>) -> tensor<1xf32> {
