...
// MemRefTableReader encapsulates the logic for reading MemRef data from a Lua table
...
public:
  ...
  void nextMemRef(int offset) {
    // The index 3 represents the fixed-size part of each MemRef entry (allocPtr, alignedPtr, offset)
    mIndex += offset;
  }
...
getScalarValue(const sol::protected_function_result &pfr, int index,
  ...
  case ScalarTypeCode::i32:
  case ScalarTypeCode::i64:
    // All integer types are stored as int64_t and later interpreted based on their ScalarTypeCode
