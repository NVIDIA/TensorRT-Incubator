...
public:
  ...
  void *reallocateOutputAsync(const char *name, void *memory, uint64_t size,
    ...
    if (size > mOutputSize) {
      ...
      if (mOutputPtr) {
        MTRT_DBGF("tensorrt module output allocator deallocating 0x%lx", mOutputPtr);
      ...
      if (memory.isOk()) {
        ...
        MTRT_DBGF("tensorrt module output allocator allocating %lu bytes at 0x%lx", mOutputSize,
                  mOutputPtr);
      }
