...
void AllocTracker::track(PointerInfo info) {
  ...
  assert((get(info.ptr).isExternallyManaged() ||
          (get(info.ptr).isInternallyManaged() &&
           isReleasedInternally(info.ptr))) &&
         llvm::formatv("Pointer {0} is expected to be either externally "
                       "managed or internally managed and released internally",
                       info.ptr)
             .str()
             .c_str());
