...
func.func private @print_tensor(
  ...
  scf.for %i = %c0 to %c4 step %c1 {
    ...
    executor.print "[%d] = %d"(%i, %el : index, !scalar_type)
  }
  return
}
...
func.func @main() -> i32 {
  %0 = arith.constant dense<0> : tensor<4x!scalar_type>
  %fill_value = arith.constant 1 : !scalar_type
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c4 = arith.constant 4 : index
  ...
  %memref = bufferization.to_memref %1 read_only :
     tensor<4x!scalar_type> -> memref<4x!scalar_type, strided<[?], offset: ?>, #executor.memory_type<host>>
