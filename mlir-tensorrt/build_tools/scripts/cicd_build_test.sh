...
ENVIRONMENT VARIABLES:
    ...
    INSTALL_DIR        - Install directory (default: ${REPO_ROOT}/install)
...
install_python_dependencies() {
  ...
  if [[ -n "${CUDA_VERSION:-}" ]]; then
     ...
     uv sync -p 3.12 --no-install-project ${frozen_flag} --extra "cu${cuda_major}" || { log_error "uv sync failed for extra cu${cuda_major}"; deactivate >/dev/null 2>&1 || true; popd >/dev/null; return 1; }
  else
     log_info "CUDA_VERSION not set; installing extra: cpu"
    uv sync -p 3.12 --no-install-project ${frozen_flag} --extra "cpu" || { log_error "uv sync failed for extra cpu"; deactivate >/dev/null 2>&1 || true; popd >/dev/null; return 1; }
  fi
