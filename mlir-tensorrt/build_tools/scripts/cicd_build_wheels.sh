...
#     DOWNLOAD_TENSORRT_VERSION - The TensorRT version to build the wheels for (default: 10.12)
...
#
# EXAMPLES:
#     # Basic usage: build wheels for all packages and Python versions
#     ./build_tools/scripts/cicd_build_wheels.sh
#
#     # Build wheels with verbose output
#     VERBOSE=1 ./build_tools/scripts/cicd_build_wheels.sh
#
#     # Build only for Python 3.10 and 3.11
#     PYTHON_VERSIONS="3.10 3.11" ./build_tools/scripts/cicd_build_wheels.sh
#
#     # Build wheel for a single package
...
for pkg in "${PACKAGES[@]}"; do
  for pyver in "${PY_VERSIONS[@]}"; do
    uv build --wheel --out-dir="${WHEELS_DIR}" --python="${pyver}" "integrations/python/${pkg}"
  done
done
