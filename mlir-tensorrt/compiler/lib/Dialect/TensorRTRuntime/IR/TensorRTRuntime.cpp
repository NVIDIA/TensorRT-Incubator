...
static bool hasCanonicalStrides(MemRefType memrefType) {
  if (auto layoutAttr =
          memrefType.getLayout().dyn_cast_or_null<StridedLayoutAttr>()) {
    // Check if the strides are canonical
    ArrayRef<int64_t> strides = layoutAttr.getStrides();
    ArrayRef<int64_t> shape = memrefType.getShape();

    int64_t expectedStride = 1;
    for (int i = shape.size() - 1; i >= 0; --i) {
      if (strides[i] != ShapedType::kDynamic && strides[i] != expectedStride)
        return false;
  ...
  // If there's no explicit layout, assume it's canonical
  return memrefType.getLayout().isa<AffineMapAttr>();
}
...
LogicalResult EnqueueAllocOp::verify() {
  ...
  for (auto resultType : getResultTypes()) {
    ...
    if (isMemRef && !hasCanonicalStrides(dyn_cast<MemRefType>(resultType)))
