...
struct BlasRunGemmOpInterface
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
    ...
    if (!callOp.getAlgo()) {
      ...
      if (failed(sizeA))
        return failure();
      SmallVector<int64_t, 6> strideA = bufferAType.getStridesAndOffset().first;
      MemRefType bufferBType = dyn_cast<MemRefType>((*bufferB).getType());
      FailureOr<SmallVector<int64_t>> sizeB = getMemrefShape(bufferBType);
      if (failed(sizeB))
        return failure();
      SmallVector<int64_t, 6> strideB = bufferBType.getStridesAndOffset().first;
